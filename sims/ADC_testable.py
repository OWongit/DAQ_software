...
class ADS124S08:
    ...
    def __init__(self, spi_bus, spi_dev, gpiochip="/dev/gpiochip0", reset_pin=None, drdy_pin=None, start_pin=None, max_speed_hz=1_000_000):
        ...
        # if "Mock" not in spidev.__name__ and not os.path.exists(devpath):
        #
        # This is the new, more robust check:
        # We only check for the device path if we are *actually* using the real "spidev" library.
        # In mock mode, spidev.__name__ will be "Mock_spidev", so this block is skipped.
        if spidev.__name__ == "spidev" and not os.path.exists(devpath):
        ...
        if reset_pin is not None:
            # Use the mock LineSettings if in mock mode
            if spidev.__name__ == "Mock_spidev":
                out_cfg[reset_pin] = LineSettings(direction=Direction.OUTPUT, output_value=Value.ACTIVE)
            else:
    ...
    def wait_drdy(self, timeout_s=0.2):
        ...
        if self._req_in is None or self.drdy_pin is None:
            # In mock mode, don't sleep for real, just a tiny bit
            if spidev.__name__ == "Mock_spidev":
                time.sleep(0.001)
            else:
